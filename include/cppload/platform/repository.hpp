...
public:
    ...
    virtual auto get_project(const std::string& id) const -> Result<Project, PlatformError> = 0;
    virtual auto list_projects() const -> std::vector<Project> = 0;
    ...
    virtual auto get_scenario(const std::string& id) const -> Result<Scenario, PlatformError> = 0;
    virtual auto list_scenarios(const std::string& project_id) const -> std::vector<Scenario> = 0;

    virtual auto create_run(std::string project_id, std::string scenario_id, std::string environment) -> Result<Run, PlatformError> = 0;
    virtual auto get_run(const std::string& id) const -> Result<Run, PlatformError> = 0;
    virtual auto list_runs(const std::string& project_id) const -> std::vector<Run> = 0;
    ...
    virtual auto get_policy(const std::string& id) const -> Result<Policy, PlatformError> = 0;
    virtual auto list_policies(const std::string& project_id) const -> std::vector<Policy> = 0;
};
